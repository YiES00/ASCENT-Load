...
def write_force_cards(
    ...
    with open(filepath, 'w') as f:
        ...
        # 이 파일은 실행 덱이 아니라 하중 카드 묶음이다. 그대로 솔버에
        # 넣으면 SOL/CEND/BEGIN BULK 가 없어 실패한다 (MSC: UWM 333 +
        # UIM 480). 실제로 이 파일을 MSC 에 제출한 사고가 있었으므로
        # 파일이 스스로 용도를 밝히게 한다.
        f.write("$ NOT A RUNNABLE DECK -- FORCE cards only, no SOL/CEND/\n")
        f.write("$ BEGIN BULK. INCLUDE this file from an analysis deck and\n")
        f.write(f"$ select the load set with LOAD = {load_sid}.\n")
        f.write("$\n")
