...
(2026-09-14) 본문을 main() 으로 감쌌다. 모듈 수준에서 파이프라인을 돌리면
macOS spawn 워커가 __main__ 을 재임포트하면서 파이프라인을 반복 실행했다.
"""
...
def main() -> None:
    ...
    from compare_hull_selection import exceedance, run_selection
    ...
    with open(os.path.join(HERE, "hull_comparison_94.json"), "w") as f:
        json.dump({"design_2d": sorted(s2), "design_3d": sorted(s3),
                   ...
                   "n_critical": {
                       "2d": len(proc2.get_critical_cases()),
                       "3d": len(proc3.get_critical_cases())}}, f,
